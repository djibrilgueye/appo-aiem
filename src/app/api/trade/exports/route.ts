...
const exportSchema = z.object({
  ...
  mainDestinations: z.string().default("[]"),
})
...
export async function GET(req: Request) {
  try {
    ...
    const exports = await prisma.tradeExport.findMany({
...
export async function POST(req: Request) {
  try {
    ...
    const record = await prisma.tradeExport.create({
      data,
      include: { country: { select: { name: true, code: true } } },
    })
    createAuditLog({ ...getAuditContext(session, req), action: "CREATE", entity: "TradeExport", entityId: record.id, description: `Created trade export for ${record.countryId} ${record.year}` }).catch(console.error)
