...
export default function EditStoragePage() {
    ...
    fetch(`/api/storage/${id}`)
      ...
      .then(d => {
        setForm({
          ...
          lat: d.lat || 0,
          lon: d.lon || 0,
          type: d.type || "Crude Oil",
          lngSubtype: d.lngSubtype || "",
          capacityMb: d.capacityMb || 0,
          regasCapacity: d.regasCapacity != null ? String(d.regasCapacity) : "",
          liquefCapacity: d.liquefCapacity != null ? String(d.liquefCapacity) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/storage/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#0D2840]">{t.admin.storage.editTitle}</h1>
      </div>
      <form onSubmit={handleSubmit} className="bg-white border border-[#D0E4F0] rounded-xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[#1B4F72] text-sm mb-1">Storage ID</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[#1B4F72] text-sm mb-1">{t.admin.storage.capacityTotal}</label>
