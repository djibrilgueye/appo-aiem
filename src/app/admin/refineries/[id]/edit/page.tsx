...
export default function EditRefineryPage() {
  ...
  useEffect(() => {
    if (!id) return
    fetch(`/api/refineries/${id}`)
      .then(r => r.json())
      .then(d => {
        setForm({
          refineryId: d.refineryId || "",
          name: d.name || "",
          countryId: d.countryId || "",
          lat: d.lat || 0,
          lon: d.lon || 0,
          capacityKbd: d.capacityKbd || 0,
          status: d.status || "operational",
          year: d.year != null ? String(d.year) : "",
        })
        setFetching(false)
      })
      .catch(() => { setError("Failed to load refinery"); setFetching(false) })
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/refineries/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
  ...
  return (
    <div>
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold text-[#0D2840]">Edit Refinery</h1>
        </div>
