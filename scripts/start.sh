#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# start.sh  —  Lance le service email puis Next.js (mode production).
...
echo "[aiem] Démarrage de Next.js (production) sur le port ${PORT}…"
cd "$ROOT" && PORT="$PORT" npx next start
